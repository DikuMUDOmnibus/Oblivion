...
printobj (FILE*, OBJ_RESET*, where, sh_int, sh_int, sh_int, sh_int, sh_int);
...
case ITEM_POTION: /* pill|scroll|potion, spell-level, 0, spell-name(s) */
	...
	for (i = 0; i < 4; i++) /* extra slot for ROM */
	{
...
void printarea (FILE *fp, AREA_DATA *pArea)
{
	...
	for(vnumparser=pArea->lvnum; vnumparser<=pArea->uvnum; vnumparser++)
	{
	parseroom=get_room_index(vnumparser);
	for (pReset = parseroom->reset_first; pReset != NULL; pReset = pReset->next)
	{
