...
void do_guild( CHAR_DATA *ch, char *argument )
{
	...
	return;
	 ...
	 if ((rank = clan_rlookup(arg3, clan)) == 0)
	 {
	 send_to_char("No such rank exists in that clan.\n\r",ch);
	 return ;
	 }

	 {
	sprintf(buf,"They are now a %s of clan %s.\n\r",
		 clan_table[clan].clan_rank_title[rank],
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "         St   Qu   Pr   Em   In   Co   Ag   Sd   Me   In\n\r");
    send_to_char( buf, ch);

    sprintf( buf, "Current  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d\n\r",
             victim->perm_stats[STAT_ST],
             victim->perm_stats[STAT_QU],
	     victim->perm_stats[STAT_PR],
	     victim->perm_stats[STAT_EM],
	     victim->perm_stats[STAT_IN],
	     victim->perm_stats[STAT_CO],
             victim->perm_stats[STAT_AG],
             victim->perm_stats[STAT_SD],
             victim->perm_stats[STAT_ME],
             victim->perm_stats[STAT_RE]);
    send_to_char( buf , ch );

    if(!IS_NPC(victim))
    {       
    sprintf( buf, "Potent   %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d  %3d\n\r",
      victim->potential_stats[STAT_ST],
      victim->potential_stats[STAT_QU],
      victim->potential_stats[STAT_PR],
      victim->potential_stats[STAT_EM],
      victim->potential_stats[STAT_IN],
      victim->potential_stats[STAT_CO],
      victim->potential_stats[STAT_AG],
      victim->potential_stats[STAT_SD],
      victim->potential_stats[STAT_ME],
      victim->potential_stats[STAT_RE]
      );
    send_to_char(buf, ch );
    }

    sprintf( buf, "Modifier %+3d  %+3d  %+3d  %+3d  %+3d  %+3d  %+3d  %+3d  %+3d  %+3d\n\r",
      victim->modifiers[STAT_ST],
      victim->modifiers[STAT_QU],
      victim->modifiers[STAT_PR],
      victim->modifiers[STAT_EM],
      victim->modifiers[STAT_IN],
      victim->modifiers[STAT_CO],
      victim->modifiers[STAT_AG],
      victim->modifiers[STAT_SD],
      victim->modifiers[STAT_ME],
      victim->modifiers[STAT_RE]
           );
    send_to_char(buf,ch);
	 
    sprintf( buf,
	"Lv: %d  Class: %s  Align: %+d/%+d  Gold: %ld  Silver: %ld  Exp: %d\n\r",
	victim->level,       
	IS_NPC(victim) ? "mobile" : class_table[victim->class].name,            
	victim->tendency, victim->alliance,
	...
	if(victim->pcdata->sactext)
	{
	  sprintf(buf, "Sacrafice: %s", victim->pcdata->sactext);
	  send_to_char( buf, ch );
	}
    }
    ...
    sprintf( buf, "Pkills: %d  PkDefeats: %d  Pksafe: %d  PkOOL: %d  PkNorecall: %d\n\r",
             victim->pkkills, victim->pkdefeats, victim->pksafe, 
             victim->pkool, victim->pknorecall);
    send_to_char(buf,ch);

    if(!IS_NPC(victim))		
    {	    
	 sprintf( buf, "Security: %d Tired:%d \n\r", victim->pcdata->security,
		victim->pcdata->tired );	/* OLC */
...
void do_snoop( CHAR_DATA *ch, char *argument )
{
    ...
    &&  room_is_private(victim->in_room) && !IS_TRUSTED(ch,CREATOR))
    {
...
void do_update( CHAR_DATA *ch, char *argument )
{
  ...
  {
    send_to_char( "Syntax: update <char> <times>\n\r", ch);
    return ;
  }
...
void do_xpen( CHAR_DATA *ch, char *argument )
{
	...
	return;
	 ...
	 if(is_number(arg2))
		if(atoi(arg2)>1 && !IS_NPC(victim))
			victim->pcdata->xp_penalty=atoi(arg2);
		else
			send_to_char("Syntax: xpen <character> <hours>\n\r",ch);
    return;
}
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "    sex class level temple\n\r",	ch );
	send_to_char( "    race group gold silver hp mana move prac\n\r",ch);
	send_to_char( "    train thirst hunger drunk full security\n\r",	ch );
	send_to_char( "    alliance tendency\n\r",	ch );
        send_to_char( "    st  qu  pr  em  in  co  ag  sd  me  re\n\r",ch);
        send_to_char( "    pst pqu ppr pem pin pco pag psd pme pre\n\r",ch );
	return;
	 }
     ...
     */
        ...
        {
                 send_to_char( "Stat range is 1 to 100.\n\r", ch );
                 return;
        }
	...
	if ( value < -3 || value > 3 )
	{
		 send_to_char( "Alliance range is -3 to 3.\n\r", ch );
		 return;
	}
	...
	if ( value < -3 || value > 3 )
	{
		 send_to_char( "Tendency range is -3 to 3.\n\r", ch );
		 return;
	}
...
void do_setsac ( CHAR_DATA *ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    {
	send_to_char("Syntax: setsac <character> <text>\n\r",ch);
	return ;
    }
    ...
    free_string( victim->pcdata->sactext );
    victim->pcdata->sactext = str_dup(argument);
...
void do_pinfo( CHAR_DATA *ch, char *argument )
{
	 ...
	 * Format it up.
	 */
	sprintf( buf, "%s[%1s %3d %+2d/%+2d %6s %3s %3dHp %3dMn %3dPks %3dPds] %s\n\r",
                 (IS_SET(ch->act, PLR_COLOR) && wch->pcdata->diplomacy) ? FG_LT_RED : FG_LT_GRAY,
		 (wch->pcdata->true_sex==1) ? "M" : "F",
                 wch->level,
		 wch->tendency,
		 wch->alliance,
		 wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name
					 : "     ",
		 class,
		 (int) wch->hit*100/wch->max_hit,
		 (int) wch->mana*100/wch->max_mana,
		 wch->pkkills,
		 wch->pkdefeats,
		 wch->name);
