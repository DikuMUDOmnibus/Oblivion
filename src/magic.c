...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if( IS_CHAT(ch) )
    { send_to_char("You may not cast spells in the chat area.\n\r",ch);	 return ; }
    ...
    || sn<skill_lookup("acid blast") || sn>skill_lookup("wrath") )
    {
...
void spell_bless( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    victim = (CHAR_DATA *) vo;
        ...
        if(is_affected(victim,skill_lookup("wrath")))
        if(check_dispel(level,victim,skill_lookup("wrath")))
        {
          return ;
        }
        else
...
void spell_cancellation( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	found = TRUE;
	  if (check_dispel(level,victim,skill_lookup("resist poison")))
	found = TRUE;
	  if (check_dispel(level,victim,skill_lookup("poison immunity")))
	found = TRUE;

	  if (check_dispel(level,victim,skill_lookup("prismatic spray")))
	found = TRUE;

	  if (check_dispel(level,victim,skill_lookup("illusions granduer")))
	found = TRUE;

	  if (check_dispel(level,victim,skill_lookup("protection fire")))
	found = TRUE;

	 if (check_dispel(level,victim,skill_lookup("protection cold")))
	found = TRUE;

	 if (check_dispel(level,victim,skill_lookup("protection lightning")))
	found = TRUE;
	...
	found = TRUE;
	 ...
	 if (check_dispel(level,victim,skill_lookup("animal essence")))
	found = TRUE;
...
void spell_curse( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (IS_AFFECTED(victim,AFF_CURSE) || saves_spell(level,victim,DAM_NEGATIVE)
        ||victim->class==5) /* fails if paladin */
	return;
	 af.where     = TO_AFFECTS;
    af.type      = sn;
    af.level     = level;
...
void spell_frostball ( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	CHAR_DATA	*victim = (CHAR_DATA *) vo;
	int			dam;
   ...
   &&  !IS_SET(victim->imm_flags,IMM_FIRE))
	{
		...
		if (obj_lose->wear_loc != -1) /* remove the item */
		{
		    ...
		    &&  (obj_lose->weight / 10) < 
			number_range(1,ch->modifiers[STAT_QU])
...
void spell_know_alignment(int sn,int level,CHAR_DATA *ch,void *vo,int target )
{
    ...
    char buf[MAX_STRING_LENGTH];	
                ...
                {
                case -3: sprintf(buf, "%s is very chaotic ", IS_NPC(victim) ? victim->short_descr : victim->name); break;
                case -2: sprintf(buf, "%s is fairly chaotic ",  IS_NPC(victim) ? victim->short_descr : victim->name); break;
                case -1: sprintf(buf, "%s has chaotic tendencies ",  IS_NPC(victim) ? victim->short_descr : victim->name); break;                
		case  0: sprintf(buf, "%s is balanced ",  IS_NPC(victim) ? victim->short_descr : victim->name); break;
                case  1: sprintf(buf, "%s has lawful tendencies ", IS_NPC(victim) ? victim->short_descr : victim->name); break;               
		case  2: sprintf(buf, "%s is quite lawful ", IS_NPC(victim) ? victim->short_descr : victim->name); break;
                case  3: sprintf(buf, "%s is extremely lawful ", IS_NPC(victim) ? victim->short_descr : victim->name); break;
                }
                send_to_char(buf,ch);

                switch (victim->alliance)
                {
                case -3: sprintf(buf, "and is aligned wholey with evil.\n\r"); break;
                case -2: sprintf(buf, "with many attachments to the dark forces.\n\r"); break;
                case -1: sprintf(buf, "and some alliance with evil.\n\r"); break;                
		case  0: sprintf(buf, "and indifferent.\n\r"); break;
                case  1: sprintf(buf, "with good tendencies.\n\r"); break;                
                case  2: sprintf(buf, "and has definite attachments to good.\n\r"); break;                
                case  3: sprintf(buf, "and a champion of the divine.\n\r"); break;               
		}
		send_to_char(buf,ch);
...
void spell_poison( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if ( saves_spell( level, victim,DAM_POISON) || victim->class==5 )
    {
