...
bool show_version( CHAR_DATA *ch, char *argument )
{
    send_to_char( VERSION, ch );
	 ...
	 send_to_char( " Modified by Airius.\n\r", ch );
	 send_to_char( DATE, ch );
	 send_to_char( " MA:12/21/95\n\r", ch );
...
REDIT( redit_mreset )
{
	 ...
	 if ( arg[0] == '\0' || !is_number( arg ) )
	 {
	send_to_char ( "Syntax:  mreset <vnum> <max # in room> <max # in world>\n\r", ch );
	return FALSE;
	 }
...
REDIT( redit_oreset )
{
	 ...
	 {
	send_to_char ( "Syntax:  oreset <vnum> <limit in world> <args>\n\r", ch );
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	...
	case ITEM_CONTAINER:
		...
		"[v1] Flags:             [%s]\n\r"
		"[v2] Key:               [%d]\n\r"
		"[v3] Max Obj Weight:    [%d lbs]\n\r"
		"[v4] Weight Modifier:   [%d]\n\r",
		obj->value[0],
...
/* WEAPONS changed in ROM */
		...
		case 2:
			...
			if ( get_obj_index( atoi( argument ) )->item_type != ITEM_KEY )
			{
			 ...
			 break;
		  case 3:
					send_to_char( "MAX ITEM WEIGHT SET.\n\r\n\r", ch );
					pObj->value[3] = atoi( argument );
					break;
		  case 4:
				send_to_char( "WEIGHT MODIFIER SET.\n\r\n\r", ch );
					pObj->value[4] = atoi( argument );
					break;
		 }
		 break;
		 ...
		 switch ( value_num )
		 {
			 ...
			 do_help( ch, "ITEM_FOOD" );
			  ...
			  case 0:
					...
					break;
			  case 1:
					send_to_char( "QUALITY OF FOOD SET.\n\r\n\r", ch );
					pObj->value[1] = atoi( argument );
					break;
			  case 3:
					send_to_char( "POISON VALUE TOGGLED.\n\r\n\r", ch );
...
OEDIT( oedit_relative_size )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
        send_to_char( "Syntax:  rsize [number]\n\r", ch );
        return FALSE;
    }

    pObj->relative_size = atoi( argument );

    send_to_char( "Relative size set.\n\r", ch);
    return TRUE;

}

OEDIT( oedit_absolute_size )
{
    OBJ_INDEX_DATA *pObj;
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
        send_to_char( "Syntax:  asize [number]\n\r", ch );
        return FALSE;
    }

    pObj->absolute_size = atoi( argument );

    send_to_char( "Absolute size set.\n\r", ch);
    return TRUE;
...
MEDIT( medit_show )
{
    ...
    sprintf( buf,
	"Level:       [%2d]\n\rAlign:       [%+d/%+d]\n\r",
	pMob->level,      pMob->tendency, pMob->alliance );
	 send_to_char( buf, ch );
...
MEDIT( medit_align )
{
    ...
    {
	send_to_char( "Syntax:  alignment [tendency] [alliance]\n\r", ch );
	return FALSE;
    }
 
    pMob->tendency = atoi( arg1 );
    pMob->alliance = atoi( argument );
