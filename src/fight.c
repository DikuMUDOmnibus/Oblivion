...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
	 ...
	 {
	 ch->pknorecall=20;
	 victim->pknorecall=20;   /*placed here for conveniance */
	 }
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, bool second )
{
	...
	if ( wield != NULL )
	{
	 ...
	 if ( IS_GOOD(victim) && IS_WEAPON_STAT(wield,WEAPON_HOLY)
	 && IS_OBJ_STAT(wield,ITEM_ANTI_GOOD) && ch->class==5)
		  dam = dam + (victim->alliance/3)*(dam*1.25);

	 if ( IS_EVIL(victim) && IS_WEAPON_STAT(wield,WEAPON_HOLY)
	 && IS_OBJ_STAT(wield,ITEM_ANTI_EVIL) && ch->class==5)
		dam = dam + (abs(victim->alliance)/3)*(dam*1.25);
	...
	if (ch->fighting == victim && IS_WEAPON_STAT(wield,WEAPON_POISON))
	{
	    ...
	    if (!saves_spell(level / 2,victim,DAM_POISON) && victim->class!=5) 
	    {
	    ...
	    bool show) 
{
    ...
    if(number_percent()<(dam*50)/victim->max_hit+ch->hitroll-20)
         {
      /*  perform_critical(ch, victim, dam*100/victim->max_hit, dam_type);*/
         }
	...
	if (!IS_IMMORTAL(ch))
	{
     ...
     switch(UMIN(number_range(1,20),number_range(1,20)))
     {
       ...
       case 11:
       case 12:  
       case 13: dam = dam / (victim->level*2/5+1);
                if(ch->level>victim->level)
                  dam=dam * (ch->level-victim->level+4)/4; break;
       case 14:
       case 15: 
       case 16:
       case 17: dam = dam / (victim->level*2/5+1);
                if(ch->level*11/10>victim->level)
                  dam=dam * (ch->level*11/10-victim->level+4)/4; break;
       case 18:
       case 19: dam = dam / (victim->level*2/5+1);
                if(ch->level*12/10>victim->level)
                  dam=dam * (ch->level*12/10-victim->level+4)/4; break;
       case 20: dam = dam / (victim->level*2/5+1);
                if(ch->level*15/10>victim->level)
                  dam=dam * (ch->level*15/10-victim->level+4)/4; break;
...
*/
	...
	victim->hit = 1;
	 ...
	 if(dam>1)
	 {
		if(is_affected(victim,skill_lookup("fire shield")))
			damage_old(victim,ch,dam/2,gsn_fire_shield,DAM_FIRE,TRUE);
		if(is_affected(victim,skill_lookup("chill shield")))
			damage_old(victim,ch,dam/2,gsn_chill_shield,DAM_COLD,TRUE);
	 }
	 update_pos(ch);
	...
	if(IS_SET(victim->act, PLR_COLOR))
		sprintf(buf, "You have been %sKILLED!!%s\n\r\n\r", FG_RED, FG_LT_GRAY);
	else
		strcpy(buf, "You have been KILLED!!\n\r\n\r");
	...
	{
		if(IS_SET(victim->act, PLR_COLOR))
			sprintf(buf, "%sThat really did %sHURT!%s\n\r", FG_YELLOW, FG_RED,
						FG_LT_GRAY);
		else
			strcpy(buf, "That really did HURT!");
		send_to_char( buf, victim );
	}
	if ( victim->hit < victim->max_hit / 4 )
	{
		if(IS_SET(victim->act, PLR_COLOR))
			sprintf(buf, "%sYou sure are %sBLEEDING!%s\n\r", FG_YELLOW, FG_RED,
						FG_LT_GRAY);
		else
			strcpy(buf, "You sure are BLEEDING!!!");
		send_to_char( buf, victim );
	}
	break;
	 }
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	{
		 send_to_char("I don't think Airius would approve.\n\r",ch);
		 return TRUE;
	}
	...
	else
	{
		if(ch->level<11)
		{
			send_to_char("You can not player kill till level 11.\n\r", ch);
			return TRUE;
		}
		if(((victim->level<11))&&(ch->pkool>0))
		{
			send_to_char("You cannot kill the weak so soon after doing so.\n\r" , ch);
			return TRUE;
		}
		if(victim->pksafe>0)
		{
			send_to_char("He has died too recently, leave him alone.\n\r", ch);
			return TRUE;
		}
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	return FALSE;
	 ...
	 if(IS_SET(victim->act, PLR_COLOR))
		sprintf(buf, "%sYou parry $n's attack.%s", FG_BLUE, FG_LT_GRAY);
	 else
		strcpy(buf, "You parry $n's attack.");
	 act( buf, ch, NULL, victim, TO_VICT    );
	 if(IS_SET(ch->act, PLR_COLOR))
		sprintf(buf, "%s$n parries your attack.%s", FG_BLUE, FG_LT_GRAY);
	 else
		strcpy(buf, "$n parries your attack.");
...
bool check_shield_block( CHAR_DATA *ch, CHAR_DATA *victim )
{
	 ...
	 if(IS_SET(victim->act, PLR_COLOR))
		sprintf(buf, "%sYou block $n's attack with your shield.%s", FG_BLUE, FG_LT_GRAY);
	 else
		strcpy(buf, "You block $n's attack with your shield.");
	 act( buf, ch, NULL, victim, TO_VICT    );
	 if(IS_SET(ch->act, PLR_COLOR))
		sprintf(buf, "%s$n blocks your attack with a shield.%s", FG_BLUE, FG_LT_GRAY);
	 else
		strcpy(buf, "$n blocks your attack with a shield.");
	 act( buf, victim, NULL, ch, TO_VICT    );
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	chance /= 2;
	 ...
	 if(IS_SET(victim->act, PLR_COLOR))
		sprintf(buf, "%sYou dodge $n's attack.%s", FG_BLUE, FG_LT_GRAY);
	 else
		strcpy(buf, "You dodge $n's attack.");
	 act( buf, ch, NULL, victim, TO_VICT    );
	 if(IS_SET(ch->act, PLR_COLOR))
		sprintf(buf, "%s$n dodges your attack.%s", FG_BLUE, FG_LT_GRAY);
	 else
		strcpy(buf, "$n dodges your attack.");
	 act( buf, victim, NULL, ch, TO_VICT    );
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
	...
	xp = xp * (2 + abs(gch->alliance-victim->alliance) +
                   abs(gch->tendency-victim->tendency)) / 9;

	 /* more exp at the low levels */
	 if (gch->level < 6)
		xp = 10 * xp / (gch->level + 2);

	 /* less at high */
	 if (gch->level > 55 )
	xp =  15 * xp / (gch->level - 25 );

/* WWW, makes xp based on hours since level, unnaffect newbies */
	diff=(int) ((gch->played + current_time - gch->logon) / 3600)-
			gch->pcdata->last_level;
	if(gch->level>15)
	{
	if(diff<1)
		xp=xp/3;
	...
	if(diff<4)
	 ...
	 /* adjust for grouping */
	 xp = ( 6 * xp * gch->level/( UMAX(1,total_levels -1) ) ) / 5;
	 ...
	 if(gch->level<5)
		...
		xp=xp*1.4;
	 else if(gch->level<20)
		xp=xp*1.2;

	 if(!IS_NPC(gch))
		if(gch->pcdata->xp_penalty>0)
			xp=0;

	if((xp>150-gch->level) && (gch->level>10) && (victim->level>12))
	{
	sprintf(buf, "BIG EXP: %s gained %d xp by killing %s [vnum: %d]",
		gch->name,
		xp,
		victim->short_descr,
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
	...
	act(buf2,ch,NULL,NULL,TO_CHAR);
	 ...
	 else
	 {
		...
		if(IS_SET(ch->act, PLR_COLOR))
			sprintf(bf,"%s%s%s", FG_GREEN, buf2, FG_LT_GRAY);
		else
			strcpy(bf, buf2);
		act( bf, ch, NULL, victim, TO_CHAR );
		if(IS_SET(victim->act, PLR_COLOR))
			sprintf(bf,"%s%s%s", FG_LT_RED, buf3, FG_LT_GRAY);
		else
			strcpy(bf, buf3);
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	return;
	 ...
	 if( IS_AFFECTED(ch,AFF_SLOW) && number_range(1,4) < 4)
	 {
		send_to_char("You feel to lethargic to run./n/r",ch);
		return ;
	 }
