...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
    ...
    if ( !str_cmp( arg, "alter" ) && IS_IMMORTAL(ch) )
    {
        if(ch->pnote==NULL)
	{
	   send_to_char("You must start a note before altering it.\n\r",ch);
           return ;
	}
        if (ch->pnote->type != type)
        {
                send_to_char( "You might want to use the note type you started with.\n\r",ch);
                return ;
        }
