...
void do_donate( CHAR_DATA *ch, char *argument )
{
	...
	sprintf(buf, "%d put %s pit", ROOM_VNUM_ALTAR, argument);
	do_at(ch,buf);
	return ;
}
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
	...
	return;
	 ...
	 if(IS_SET(obj->extra_flags, ITEM_MAGIC)&&ch->class==7)
	 {
	...
	{
		 send_to_char("Your hands are tied up with your two-handed weapon!\n\r",ch);
		 return;
	}
	 ...
	 if (get_eq_char (ch, WEAR_SECONDARY) != NULL)
	  ...
	  if(is_affected(ch,skill_lookup("broken shieldarm")))
	  {
		send_to_char("You cannot use a shield while your arm is broken.\n\r",ch);
		return ;
	  }
    ...
    if ( CAN_WEAR( obj, ITEM_HOLD ) )
	 {
	  ...
	  if(is_affected(ch,skill_lookup("broken shieldarm")))
	  {
		send_to_char("You cannot hold an item while your arm is broken.\n\r",ch);
		return ;
	  }
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "$n offers $mself to Takhisis, who declines with haste.",
	    ch, NULL, NULL, TO_ROOM );
	send_to_char(
	    "Takhisis does not appreciate your offer.\n\r", ch );
	return;
    }
    ...
    silver = UMAX(1,obj->level * 3);
	 ...
	 if (silver == 1)
	{
        sprintf(buf1,"%s gives you one silver coin for your sacrifice.\n\r",
		(ch->pcdata->sactext) ? ch->pcdata->sactext : "A omniscient power" );
        send_to_char(buf1,ch);
	}
	 else
	 {
	sprintf(buf1,"%s gives you %d silver coins for your sacrifice.\n\r",
           (ch->pcdata->sactext) ? ch->pcdata->sactext : "A omniscient power",
            silver );
        send_to_char(buf1,ch);
    }
	...
	if(ch->pcdata->sactext)
	 act( "$n sacrifices $p to $T.", ch, obj, ch->pcdata->sactext, TO_ROOM );	
	else
	  act( "$n sacrifices $p to a great power.", ch, obj, NULL, TO_ROOM );
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    percent  += ( IS_AWAKE(victim) ? 10 : -50 );
	 ...
	 if ( !IS_NPC(ch) && (percent+2*victim->level-2*ch->level > get_skill(ch,gsn_steal)))
	 {
	/*
	 * Failure.
	 */
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	if (( obj->level > ch->level ) && ( obj->level >10))
	{
...
/* wear object as a secondary weapon */
{
	...
	if(obj->item_type != ITEM_WEAPON)
	{
	 ...
	 if(!CAN_WEAR( obj, ITEM_WIELD ))
	 {
		send_to_char("You can not wield that./n/r",ch);
		return ;
	 }
...
/* check for str - secondary weapons have to be lighter */
	 ...
	 if (IS_WEAPON_STAT(get_eq_char(ch,WEAR_WIELD),WEAPON_TWO_HANDS)
	     && ch->size < SIZE_LARGE)
	 {
		send_to_char( "You cannot wear a weapon in your off hand if using a two handed weapon.\n\r",ch);
		return ;
	 }
...
/* at last - the char uses the weapon */
	 ...
	 {
		send_to_char( "You can't use a two handed weapon in your off hand.\n\r",ch);
		return ;
	 }
