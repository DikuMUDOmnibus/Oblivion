...
/*  {   command		function	}, */
    ...
    {   "?",		show_help	},
    {   "version",	show_version	},
    {   "rsize", oedit_relative_size },
    {   "asize", oedit_absolute_size },	
...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char("You must have at least a security rating of 6 to edit areas.",ch);
		return ;
	}
	...
	if ( !str_cmp( argument, "create" ) )
	{
		...
		{
		send_to_char("You must have at least a security rating of 8 to create areas.",ch);
		return ;
		}
...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char("You must have at least a security rating of 1 to edit rooms.",ch);
		return ;
	}
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char("You must have at least a security rating of 2 to edit objects.",ch);
		return ;
	}
...
void do_medit( CHAR_DATA *ch, char *argument )
{
    ...
    if(ch->pcdata->security<2)
	{
		send_to_char("You must have at least a security rating of 2 to edit mobiles.",ch);
		return ;
	}
...
void do_resets( CHAR_DATA *ch, char *argument )
{
	...
	{
	default :
		send_to_char( "Syntax: RESET <number> M <mob vnum> <max world> <max room>\n\r", ch );
		send_to_char( "        RESET <number> O <vnum>\n\r", ch );
		send_to_char( "        RESET <number> P <obj vnum> <max world> <into vnum> <number loaded>\n\r", ch );
		send_to_char( "        RESET <number> G <obj vnum> <max #>\n\r", ch );
		send_to_char( "        RESET <number> E <obj vnum> <max #> <wear loc>\n\r", ch);
		send_to_char( "        RESET <number> delete   <-- to delete a reset\n\r", ch );
		break ;
	 case 'm':
	 case 'M':
	 ...
	 case 'P':
		 ...
		 if(is_number(arg5))
		 {
			if(get_obj_index(atoi(arg5))!=NULL)
				pReset->arg3=atoi(arg5);
			else break ;
		 } else break ;
		 if(is_number(arg6))
		 {
			 pReset->arg4=atoi(arg6);
		 } else break ;
		 add_reset( ch->in_room, pReset, atoi( arg1 ) );
		 send_to_char( "Reset added.\n\r", ch );
		 break ;
	 ...
	 case 'E':
		 ...
		 if ( flag_value( wear_loc_flags, arg5 ) == NO_FLAG )
		 {
			send_to_char( "Resets: '? wear-loc'\n\r", ch );
			return;
		 }
		 pReset->arg3 = flag_value( wear_loc_flags, arg5 );
		 add_reset( ch->in_room, pReset, atoi( arg1 ) );
		 send_to_char( "Reset added.\n\r", ch );
		 break ;
	}
