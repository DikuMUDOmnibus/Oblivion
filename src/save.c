...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	fprintf( fp, "LnD  %s~\n",	ch->long_descr	);
	 ...
	 if ((ch->clanrank>0)&&(ch->clan>0))
		fprintf( fp, "Rank %s~\n",clan_table[ch->clan].clan_rank_title[ch->clanrank]);
	 if (ch->pkool)
	 fprintf( fp, "PKool  %d\n", ch->pkool );
	 if (ch->pksafe)
	 fprintf( fp, "PKsafe %d\n", ch->pksafe );
	 ...
	 if(!IS_NPC(ch))
	 {
		...
		if(ch->pcdata->diplomacy)
		 fprintf(fp, "Diplo  %d\n", ch->pcdata->diplomacy );
                if(ch->pcdata->diplomacy && ch->pcdata->dip_points)
	...
	ch->modifiers[STAT_IN],
        ...
        fprintf (fp, "APot %d %d %d %d %d %d %d %d %d %d\n",
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'L':
		 KEY( "LastLevel",	ch->pcdata->last_level, fread_number( fp ) );
                 KEY( "Lang", ch->language, fread_number( fp ) ); 
	...
	case 'R':
		 ...
		 KEY( "Rank", ch->clanrank, clan_rlookup(fread_string(fp), ch->clan));
	...
	case 'S':
		 KEY( "Sactx", ch->pcdata->sactext, fread_string( fp ) );
		...
		if ( sn < 0 )
		{
		  ...
		  if((ch->version<=6)&&(sn>=skill_lookup("acid blast"))&&
	...
	case 'T':
		...
		KEY("Tired", ch->pcdata->tired, fread_number( fp ) );
		 ...
		 KEY( "Trying", ch->pcdata->is_trying_sleep, fread_number( fp ) );
