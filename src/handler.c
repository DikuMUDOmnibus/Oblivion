...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	else if (sn == gsn_quad_attack && IS_SET(ch->act,ACT_WARRIOR))
	    skill = 5 * ch->level - 60;	
...
void reset_char(CHAR_DATA *ch)
{
    ...
    {
       ch->modifiers[stat]=0;
       if(ch->perm_stats[stat] > 70)
          ch->modifiers[stat] = ch->perm_stats[stat]-70;
       else
       if(ch->perm_stats[stat] < 26)
          ch->modifiers[stat] = -26 + ch->perm_stats[stat];
       ch->modifiers[stat] += pc_race_table[ch->race].stat_bonus[stat];
    }
...
int can_carry_n( CHAR_DATA *ch )
{
    ...
    return MAX_WEAR +  2 * (ch->modifiers[STAT_ST]+20) + ch->level + 10;
...
int can_carry_w( CHAR_DATA *ch )
{
    ...
    return ((ch->modifiers[STAT_ST]+20) * 10 + ch->level * 20)*10+500;
}
...
/* RT changed so that WIZ_INVIS has levels */
	...
	chance += ch->modifiers[STAT_AG] * 3/2;
 	chance -= victim->modifiers[STAT_IN] * 2;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_ST:		return "strength";
    case APPLY_QU:		return "quickness";
    case APPLY_PR:		return "presence";
    case APPLY_EM:		return "empathy";
    case APPLY_IN:		return "intuition";
    case APPLY_CO:              return "constitution";
    case APPLY_AG:              return "agility";
    case APPLY_SD:              return "self-discipline";
    case APPLY_ME:              return "memory";
    case APPLY_RE:              return "reasoning";
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_CHAOS        ) strcat( buf, " chaos"        );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
