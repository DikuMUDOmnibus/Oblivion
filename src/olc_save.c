 ...
 */
	 ...
	 switch ( pObjIndex->item_type )
	 {
		 ...
		 fprintf( fp, "%s %d %d %s %s\n",
					 weapon_table[pObjIndex->value[0]].name,
					 pObjIndex->value[1],
					 pObjIndex->value[2],
	 ...
	 fprintf( fp, "%d %d\n", pObjIndex->absolute_size,
         ...
         fprintf( fp, "%s\n", fwrite_flag(pObjIndex->class_restriction, buf) );
	 ...
	 for( pAf = pObjIndex->affected; pAf; pAf = pAf->next )
	 {
		  ...
		  else
		  {
			...
			{
			 default :
			  case TO_AFFECTS: flag_letter="A"; break;
			  case TO_IMMUNE : flag_letter="I"; break;
			  case TO_RESIST : flag_letter="R"; break;
			  case TO_VULN   : flag_letter="V"; break;
			}
			fprintf( fp, "F %s %d %d %s\n",
							flag_letter,
							pAf->location,
							pAf->modifier,
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
		...
		for ( pRs = pRoomIndex->spec_data; pRs; pRs = pRs->next ) {
    fprintf( fp, "F %d %d %d %d\n%s~\n",pRs->lowhour,pRs->highhour,\
             pRs->lowroll,pRs->highroll,fix_string( pRs->special ) );
}
		...
		if(pRoomIndex->heal_rate!=100 && pRoomIndex->heal_rate!=0)
			fprintf( fp, "H %d ", pRoomIndex->heal_rate);
		if(pRoomIndex->mana_rate!=100 && pRoomIndex->mana_rate!=0)
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
        {
			...
			fprintf( fp, "D 0 %d %d %d The %s door of %s is %s\n",
				pRoomIndex->vnum,
				pExit->orig_door,
				pExit->rs_flags==2 ? 2 : 1,
				dir_name[ pExit->orig_door ],
				pRoomIndex->name,
				pExit->rs_flags==2 ? "closed and locked"
					 : "closed" );
#endif
#if !defined( VERBOSE )
